...
\begin{itemize}[label=-]
    ...
    \vspace{-10pt}
    \item \textbf{Xác thực và phân quyền:} Hệ thống phải hỗ trợ xác thực mạnh (SSO cho thành viên trường, cơ chế OTP hoặc MFA khi cần) và phân quyền theo vai trò để đảm bảo chỉ người được ủy quyền mới truy cập các chức năng quản trị.
    \vspace{-10pt}
    \item \textbf{Bảo vệ dữ liệu:} Mã hóa dữ liệu nhạy cảm khi truyền tải (TLS) và khi lưu trữ theo mức độ nhạy cảm (ví dụ AES-256 cho dữ liệu quan trọng); quản lý khóa an toàn và hạn chế truy cập trực tiếp đến kho dữ liệu.
\end{itemize}